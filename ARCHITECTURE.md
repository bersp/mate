...
- `Slide` holds `elements` (root list) plus `title` / `subtitle` references. `add(element)` appends and returns it for chaining. Root elements keep `parent = None` (the slide is not itself an `Element`). A slide is built mutable and sealed with `close()`, which snapshots its rendered Typst fragment into `_fragment` (an opaque backend artifact the core never inspects; `is_closed` reflects it). Mutations after `close()` do not affect the snapshot. `close()` stays backend-agnostic — it routes through `Presentation._render_slide`, so `Slide` never names a backend type.
