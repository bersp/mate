...
| `z_order` | Draw order among the elements placed on a slide: a higher value covers a lower one, and equal values keep the order the elements were added in. `None` (default) inherits from the nearest ancestor that carries one, resolved by `get_effective_z_order()`. |
...
| `scale_factor` | Uniform scale of the element's own body about its centre. The backend emits the matching `#scale`; the measured bbox reports the scaled size. On a `Group` (no body of its own) it records the accumulated rigid factor read by `set_scale`. |
...
Public API: `move_to / shift / next_to / set_anchor / rotate / scale / set_scale / get_bbox / get_anchor_point / get_width / get_height / get_bbox_center / copy` plus the `pos`, `anchor`, and `center` properties. The module-level `bbox_anchor_point(bbox, anchor)` resolves an anchor on any centre-based `(x, y, w, h)` box; every anchor point in the project (element, group union, region) goes through it. Movement (`move_to` / `shift`) forces `placement = "fixed"` and invalidates the bbox cache of the whole tree.
...
- `next_to(target, side, align=, gap=)` — places this element's box against one side of `target`'s box; a `VecLike` target is taken as a box of zero size. `_SIDE_CONTACTS` maps `(side, align)` to the pair of anchor points brought into contact and the direction the gap opens along; the element then travels through `move_to`, which leaves `_anchor` and `offset` untouched. `gap` defaults to the `arrange.gap` config value. Both boxes are measured in one pass when either is cold.
...
- `scale(factor, origin=None)` — scales this element and its fixed subtree rigidly by `factor` about `origin` (default: the element's own centre), compounding over repeated calls. Each `_place_roots` node multiplies its `scale_factor` and maps its centre to `origin + factor * (centre - origin)`, giving a `Group` a single rigid enlargement. A piece whose centre coincides with `origin` scales in place: its position, anchor, and placement stay untouched, and an inline run stays in flow. A piece the mapping displaces becomes `"fixed"`, re-anchored to `"center"` at its mapped point. A fragment inside a math run accumulates its factor and scales within the equation. `set_scale(factor, origin=None)` is the absolute form: it applies `factor / scale_factor`, landing on `factor` times the original size.
- `get_width()` / `get_height()` — return bbox width/height in cm; shape primitives override with their intrinsic values under the element's own transforms (no measurement): `_transformed_extents` applies the scale factor and the rotated-box extents (`w' = w|cos| + h|sin|`, `h' = w|sin| + h|cos|`); the fast path and the measured bbox agree.
...
**Cache invalidation is geometric only.** Three routines cover it, each clearing `_bbox` through `_clear_bbox_subtree`:
...
- `Presentation` is a :class:`PresentationTemplate` (see below) plus the list of `Slide`s. It owns the global `width` / `height` (every slide has the same size), publishes them to `config`, and tracks `current_slide` (the most recently created slide, set by `new_slide`). It also carries the presentation `name`; `write(path=None, ppi=None)` raises if any slide is still open, then compiles every slide's snapshot markup (flattened across slides) via `TypstRenderer.compile_document` into `path`, defaulting to `<name>.pdf`. `slide_counter` numbers the counted slides: `new_slide(counted=None)` counts every slide but a cover and gives it the counter's value as `number`, a template's hook slide passes `counted=False`, and `adjust_slide_count(delta)` moves the counter and the open slide's number with it (`> adjust slide count : -1` on a slide duplicated by hand gives it the original's number). `end_slide` builds the footer from the number that stands and places it on the first reveal step. `total_slides` is the value the counter is declared to reach (the command line passes the deck's `#` headings plus the deltas of its `> adjust slide count` lines), and `write` raises when the counter ends elsewhere: the check that catches a fence swallowing the rest of the deck.
...
A standalone drawing: a flat list of root elements compiled to a one-page document sized to their content. `add(element)` appends a root; `write(path, ppi=None)` reports the crossing boxes when `warn.collisions` is on, measures everything in one pass, translates the roots in place to centre their union bbox at the origin (idempotent: a repeated `write` translates by zero), and compiles a page of exactly the union's size via `TypstRenderer`. A figure file builds one module-level `Figure` and calls `write` under an `if __name__ == "__main__":` guard: run directly it compiles the PDF; executed under another `__name__` the guard does not fire. `mate <deck.md> --figure <file>` runs the file as `__main__` after building a `Presentation` on the deck's front matter, which applies the templates' `setup` and the deck's palette and config the way an embedding does.
...
- `Slide` is the authored unit (one ``#`` block): `title` / `subtitle` strings (passed to `new_slide`) plus `steps`, a list of reveal steps. `background` holds the element the template draws behind the slide, which `new_slide` adds as the first root; `footer` holds the footer group when the slide has one. `is_cover` marks a cover; `counted` says whether the slide takes a number (a counted slide carries the footer and counts toward the declared total), defaulting to `not is_cover`. Each step is a list of root elements; those with `placement != "fixed"` are skipped at render time. Root elements keep `parent = None` (the slide is not itself an `Element`). `add(element)` appends to the current step and returns it for chaining; `pause()` opens a new step. A slide starts with one (empty) step, so a slide without a pause is a single step. `Slide` never references a backend.
- `reveal_states()` yields `(roots, hidden_ids)` per step: the cumulative root elements visible at step `k` (an element listed in `replaced` at step `j` is dropped from every `k >= j`) and the `id()` of the nodes in `reveals` whose step has not arrived, which the renderer draws with `#hide` so a `||` tail holds its space. `Presentation.end_slide` renders one `Snapshot` from each.
...
### `core/authoring.py` — authored expressions

The Python written inside a deck (a blockquote method call's arguments, a fenced block's property text, the properties of a `[...][...]` markup span) evaluates here. `author_globals()` is the namespace they see: the public `mate` API, so any of them can name an element type, a `Gradient`, or `config`. `eval_props(source)` reads keyword text into a `name -> value` mapping and `eval_call(source, target)` calls `target` with the authored argument text.
...
**Markup:** `Text("foo [bar [baz][id=1]][id=2]")` parses balanced brackets; each block followed by a `[<props>]` pair becomes a sub-`Text` (`placement="inline"` by default) appended in source order to the parent's `subs` list, with each property applied through `apply_prop` (so `id=K` registers it in `id_registry` under the key `K`). A `[[<props>]]` anywhere in the source applies its properties to the whole block. A `**bold**` or `*italic*` pair around a span (`**a [b][id=1] c**`) becomes a `Text` of its own holding the emphasized run, with `weight="bold"` or `style="italic"` applied through the same queue as the spans' properties, which the queue applies outermost first. A span's own `weight=`/`style=` overrides the emphasis around it. The spans inside stay in the enclosing node's `subs` (an emphasis is no bracket level of its own), and emphasis with no span inside it stays inline markup on the leaf. Subs are real `Element`s: calling `move_to` (or `shift`) on a sub flips it to `placement="fixed"` and lifts it out of parent flow.

Every `Text` carries explicit `font: str` and `fontsize: float` (points) as keyword-only constructor params, defaulting to the `text.font` / `text.fontsize` config values. The backend wraps every rendered/measured Text in `#text(font: "...", size: ...pt, ...)`, built by `_text_style_attrs`. Sub-elements built by the parser inherit the parent's font/size, propagated in the constructor after `_take_children`. An optional `max_width: float | None` (cm) wraps the text: the backend boxes it at `min(natural width, max_width)` via a Typst `#context` block, so the bbox width shrinks to fit and the height grows with the wrapped line count.
...
- `Arrow(start, end, ...)` is a `Group` of the shapes it draws with: a `Line` shaft plus one `ArrowTip` per marked endpoint, `tip` at `end` and `tail` at `start`. `tail` defaults to none, `tip` to the `ARROW_TIPS` entry the `arrow.tip` config value names, validated at construction. `gap` (default `arrow.gap`) is the distance each drawn end keeps from its endpoint: `_reseat` plants the markers and the shaft on the endpoints pulled `gap` inward along the direction, and `start`/`end` stay the authored points. `_pos` is the midpoint of the endpoints, the bbox is the union of the pieces, and the backend renders each piece as the shape it is.
...
`z_order` lives on `Element` and cascades down the parent chain like `hidden`: `Element.get_effective_z_order()` returns the node's own value, else the nearest ancestor's, else `0`. A value set on a `Group` (or on an `Arrow`, whose pieces are fixed children with their own `#place` block) carries the whole subtree; a descendant that sets its own value departs from it.

`TypstRenderer.render_snapshot` emits the page's `#place` blocks in ascending effective `z_order` with a stable sort: equal values keep the order the elements were added in, a slide's roots in `add` order, later reveal steps after earlier ones, and each fixed descendant after its ancestor.

## Composition

### `composition/arrange.py` — `arrange`

`arrange(elements, pos, anchor, *, gap=0.0, width=None, direction="column")` stacks elements in a single column, top-to-bottom in list order, with bboxes flush against each other (plus `gap` between them). `direction="row"` lays them left to right instead (`_arrange_row`): the gap runs horizontally, the row's height is the tallest element's, and the vertical half of `anchor` places every element within it; `width`, `align` and `indent` play no part in a row. The stack as a whole is anchored at `pos` with mode `anchor`: the union bbox is sized as `(max(widths), sum(heights))` (conceptually) and positioned so its `anchor` point lands at `pos`. Horizontal placement is per element: each one sits at its own `align` (`"left"`/`"center"`/`"right"`) within the horizontal extent `width` (the widest element when omitted), falling back to the horizontal half of `anchor` when `align` is `None`, and displaced by its `indent`. The vertical half of `anchor` picks where `pos.y` sits relative to the stack (`top-*` → top edge, `center-*` → vertical center, `bottom-*` → bottom edge). Each element is moved via `move_to`, which honors that element's own anchor.
...
An element's `offset` (the displacement accumulated by `shift`) is added on top of the stack position, so a shifted element keeps its displacement across an `arrange`.
...
`Region` is a rectangle (centre, width, height) plus a default `anchor` and `arrange_gap`. It exposes `center`; `left`/`right`/`top`/`bottom` as the midpoint `Vec` of each edge; and `get_anchor_point(anchor)` for the nine anchor points. Slide-relative classmethods (`Region.create_full()`, `create_left(w)`, `create_right(w)`, `create_top(h)`, `create_bottom(h)`, `create_inner(*, left=, right=, top=, bottom=)`) read the slide size from `config` and build regions from it; `create_inner` takes side regions and returns what's left after subtracting each side's extent from the slide; `from_vertices(top_left, bottom_right)` builds one from two opposite corners. `adjust_borders(left=, right=, top=, bottom=)` mutates in place by moving each border outward (positive) or inward (negative); chained setters `set_width`/`set_height`/`set_center`/`set_anchor`/`set_arrange_gap` follow the same return-self pattern. `set_anchor_default(anchor)` also sets the anchor `reset_anchor()` restores when a slide opens. `grid(template, *, hgap=, vgap=, width_ratios=, height_ratios=)` splits the region and merges cells sharing a label into one sub-`Region` (sub-regions inherit the parent's `anchor` and `arrange_gap`).

Region owns its elements: `add(el)`, `remove(el)`, `replace(old, new)`, `remove_all()` mutate `region.elements`. `region.arrange()` stacks `region.elements` via `composition.arrange.arrange` using the region's own `anchor`, `arrange_gap`, and width.
...
One check per `warn.*` config key, each a `find_*` returning what it found and a `log_*` writing one warning per entry under the subject the caller names (`Slide 7`, a figure's file name).

- `find_overflows(layout, slots=())` — the regions whose arranged content is larger than the region, with the excess in cm. Reads the boxes the arrange pass measured, with no query of its own. `slots` are `(name, region, elements)` groups a region holds without stacking, one per `markdown alternate` variant (`PresentationTemplateBase._alternate_slots`, filled by `add_alternate` and cleared with `_alternates`): a `VSpace` carries the slot's height into the region's stack, so each group is measured against the region's width alone.
- `find_collisions(roots, hidden_ids)` — the pairs of drawn boxes that cross, each with the shared area, ordered by descending area. It walks the roots for the nodes the renderer places (fixed, not a `Group`, not a spacer, not hidden), grows each box by its `stroke_width` (the ink a stroke lays outside the box, which gives a horizontal line a height), and sweeps them ordered by left edge. A pair is dropped when one box holds the other, when the two are ancestor and descendant, or when both are pieces of the same `Arrow`. A pair that survives is then tested by geometry when either side is a `Line` or a `Curve` of `MoveTo`/`LineTo` steps (an open arrow marker): `_segments_of` reads it as segments, and it collides only where a segment, grown by half its stroke, enters the other's box (`_segment_crosses_box`, a Liang-Barsky clip) or crosses one of the other's segments (`_segments_cross`); a diagonal shaft passes the boxes its own box spans.
- `find_widows(elements, min_words)` — the paragraphs whose last line carries `min_words` words or fewer. A candidate is a `Text` carrying a `max_width`; verbatim runs, whole equations, a paragraph holding a hard line break and a paragraph narrower than half the slide (a grid column, where rewording does not steer the wrap) are left out. Each candidate is copied with its last `k` words removed, for `k` up to `min_words`, every copy is measured in one pass, and the smallest `k` whose removal costs the paragraph a line is the number of words its last line carries. The removal runs over the parsed inlines of each leaf: an emphasis emptied by it goes with its content, and a code or math span counts as one word.
...
`config.templates` lists template entries (e.g. `["nice"]`); `load_template` resolves each to a module's `PresentationTemplate` class — a `.py` file path is loaded directly, any other name is the built-in `mate.templates.<name>`, and a name that is neither raises listing `built_in_templates()`. The driver resolves a front-matter entry to a sibling `<name>.py` next to the Markdown file when present, else keeps it as a built-in name. Each `PresentationTemplate` subclasses the base `PresentationTemplate`. `Presentation.__new__` reads `config.templates` and builds a dynamic subclass whose bases are `(Presentation, *template_classes)`, so the MRO is `Presentation → templates… → PresentationTemplate`. A template writes its colors, config knobs and bullet symbols in `setup()`, adjusts the layout in `setup_layout()`, and overrides methods like `add_footer`. Earlier-listed templates take precedence, for the two hooks and for overridden methods alike. The `#>` directive properties a template's `on_directive` reads are declared in its `directive_properties` class attribute (name to one-line description); the base declares `cover`, `title`, `author` and `date`, `_declared_directive_properties()` returns the union over the MRO, and `mate --info` prints it as its DIRECTIVES block. `run_directive(directive)` is the deck-side entry point, the counterpart of `run_method_call` for blockquote commands: it raises on a property outside that union, naming it and listing the declared ones, then calls `on_directive`. Content methods share three public pieces a template's own command builds on: `resolve_region(name)` turns a region name into the `Region` a call lands in (honouring the `"active"` default and a fragment or overwrite body), `content_indent` is the ambient indent an element carries into `arrange` (the text column of a list item), and `dispatch_block(block)` renders a parsed block the way the deck itself does.

`PresentationTemplate.__init__` drives the construction in four steps: `_run_template_hook("setup")`, then the front matter's `config`/`colors`, then `build_layout()`, then `_run_template_hook("setup_layout")`. `_run_template_hook(name)` walks `type(self).__mro__` in reverse and calls the definition each class carries in its own `__dict__`, so the base-ward template writes first and the earliest-listed one writes last. The front matter sits between the two hooks: it overrides every key a template set, and the layout is built from the values that won. Templates define neither `__init__` nor `super()` calls, so a hook's position in a template's body carries no meaning.
...
- `_wrap_rotate(el, body)` — wraps `body` in `#box(rotate(-angle deg, origin: center + horizon, reflow: true)[...])` when `el.angle` is nonzero. The negated sign maps `el.angle` (counterclockwise, y-up) onto Typst's clockwise `#rotate`; `reflow: true` makes `measure(...)` report the rotated bounding box. The `#box` keeps the rotation inline (a bare `#rotate` is a block that breaks the surrounding paragraph): an inline run spins within its own line.
- `_wrap_scale(el, body)` — wraps `body` in `#box(scale(factor%, origin: center + horizon, reflow: true)[...])` when `el.scale_factor` is not 1; `reflow: true` makes `measure(...)` report the scaled extents. A `Group` body stays bare: its factor is rigid-scale bookkeeping and each rendered piece carries its own transform.
- `_wrap_transforms(el, body)` — composes the two visual transforms, scale then rotation, applied to every node's body in `_bare` and both `_render_node`s, and to a math fragment's body in `_math_fragment_markup` (re-entering math for the fragment's equation typesetting); measured size and rendered mark then carry the same transforms.
...
The isolated `(w, h)` of an element is a pure function of the Typst body handed to `measure(...)`, the preamble its markup is typeset under, the fonts it resolves against, the image file it draws, and the compiler version. `MeasureCache` stores those sizes on disk (`.mate_cache/measure_cache.json`) keyed by `_size_cache_key(body, font_signature, preamble, image_signature)` — a SHA-256 over the body, the font signature (path/size/mtime of every file under `_font_paths()`), the `typst.preamble` config value, the image signature (size and mtime of the file an `Image` draws, empty for every other element), the `typst` package version, and a format-version constant. A measurement pass serves every hit from the cache and emits a `measure(...)` record only for the misses; when no size misses and no inline-`x` probes remain, the pass runs no Typst query at all. The file is rewritten on interpreter exit with exactly the keys touched in the run, keeping it scoped to the current deck. Inline-`x` records are flow-dependent and are not cached.
...
- `Presentation.write` compiles to `<name>.pdf` in the cwd when given no path. The measurement path is internal to the backend (`.mate_cache/measure.typ`).
