...
Public API: `move_to / shift / set_anchor / rotate / scale / set_scale / get_bbox / get_width / get_height / get_bbox_center / copy` plus the `pos`, `anchor`, and `center` properties. Movement (`move_to` / `shift`) forces `placement = "fixed"` and invalidates the bbox cache of the whole tree.
