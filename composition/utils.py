...
def layout_to_group(
    ...
    """One sub-:class:`Group` per region: a region-sized rectangle and a label.
    ...
    """
    selected = (
        layout.regions.items() if names is None else ((n, layout.get(n)) for n in names)
    )
    sub_groups = [
        Group(
            [
                Rectangle(region.width, region.height, **drawable_kw).move_to(
                    region.center
                ),
